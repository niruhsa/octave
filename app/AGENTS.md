...
- **Album art**: best-effort, fetched client-side by replicating the server's Cover Art Archive flow ([`artwork.rs`](./src-tauri/src/downloads/artwork.rs)) — MusicBrainz release search by artist+album → `coverartarchive.org/release/{mbid}/front` → saved as `cover.jpg` next to the track, with an `album_art.local_cover_path` row. The server exposes no cover-byte endpoint (it only stamps `cover_path` server-side), so this is the only way to populate `local_cover_path` without server changes; any failure is logged and the track still downloads.
