...
async fn subscribe_flag_drives_the_offline_list() {
    ...
    repo::set_podcast_subscribed(&pool, "p2", true)
        .await
        .unwrap();
    assert_eq!(
        repo::list_subscribed_podcasts(&pool).await.unwrap().len(),
        2
    );
...
async fn episode_upsert_counts_and_bytes() {
    ...
    repo::upsert_podcast(&pool, &show("p1", 1)).await.unwrap();
    repo::upsert_episode(&pool, &episode("e1", "p1", 5_000_000))
        .await
        .unwrap();
    repo::upsert_episode(&pool, &episode("e2", "p1", 7_000_000))
        .await
        .unwrap();

    assert_eq!(
        repo::count_downloaded_episodes_for_podcast(&pool, "p1")
            .await
            .unwrap(),
        2
    );
    ...
    assert_eq!(
        repo::downloaded_episode_bytes(&pool).await.unwrap(),
        12_000_000
    );
    assert_eq!(
        repo::list_episodes_for_podcast(&pool, "p1")
            .await
            .unwrap()
            .len(),
        2
...
async fn metadata_episodes_cached_without_being_downloaded() {
    ...
    repo::upsert_episode(&pool, &episode("e1", "p1", 5_000_000))
        .await
        .unwrap();
    repo::upsert_episode_meta(&pool, &meta("e2", "p1", "guid-e2"))
        .await
        .unwrap();
    repo::upsert_episode_meta(&pool, &meta("e3", "p1", "guid-e3"))
        .await
        .unwrap();
    ...
    assert_eq!(
        repo::list_downloaded_episodes(&pool).await.unwrap().len(),
        1
    );

    // The guid snapshot (drives incremental sync) covers every cached episode.
    let guids: HashSet<String> = repo::list_episode_guids(&pool, "p1")
        .await
        .unwrap()
        .into_iter()
        .collect();
    ...
    assert!(
        e1.local_file_path.is_some(),
        "download must survive a metadata re-sync"
    );
    assert_eq!(repo::count_downloaded_episodes(&pool).await.unwrap(), 1);
    ...
    let removed = repo::delete_stale_metadata_episodes(&pool, "p1", &keep)
        .await
        .unwrap();
